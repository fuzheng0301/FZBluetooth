...
- (void)setMode:(FZHProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == FZHProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(FZHRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == FZHProgressHUDModeDeterminate) {
#if __has_feature(objc_arc)
        self.indicator = [[FZHRoundProgressView alloc] init];
#else
        self.indicator = [[[FZHRoundProgressView alloc] init] autorelease];
#endif
}
    else if (mode == FZHProgressHUDModeCustomView && self.customView != nil){
...
+ (FZHProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	FZHProgressHUD *hud = [[FZHProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[FZHProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		FZHProgressHUD *HUD = (FZHProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"FZHProgressHUDViewIsNillException" 
					format:@"The view used in the FZHProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = FZHProgressHUDAnimationFade;
        self.mode = FZHProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == FZHProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == FZHProgressHUDAnimationZoom) {
...
@implementation FZHRoundProgressView
